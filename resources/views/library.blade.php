...
@section('main__content')
    ...
    <div class="container">
        <div class="profile library">
            <a href="{{ route('book-add', ['id' => Auth::user()->id, 'book_id']) }}" class="library_a">Добавить книгу</a>
            <h1 class="profile__name">Библиотека</h1>
            <div class="profile__notes">
                @foreach($books as $book)
                    <div class="profile__note">
                        <div class="profile__title">
                            <a class="profile__book-a" href=" {{route('book-read', ['id' => Auth::user()->id, 'book_id' => $book->id])}} ">{{ $book->title }}</a>
                        </div>
                        ...
                        @if(Auth::user()->id == $book->user_id)
                            ...
                            <button class='library__button' onclick="copy('{{ route('book-read', ['id' => Auth::user()->id, 'book_id' => $book->id, 'right' => $book->local_id]) }}')">Поделиться доступом</button>
                        @endif
                    </div>
                @endforeach
            </div>
